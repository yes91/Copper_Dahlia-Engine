...
namespace tmx
{
	...
	public:
		...
		bool Load(const std::string& mapFile, EventDispatcher& dispatch);
		...
		std::vector<std::shared_ptr<GameObject>> QueryQuadTree(const sf::FloatRect& testArea);
	...
	private:
		...
		bool m_ParseObjectgroup(const pugi::xml_node& groupNode, EventDispatcher& dispatch);
