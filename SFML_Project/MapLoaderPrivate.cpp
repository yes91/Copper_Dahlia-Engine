...
bool MapLoader::m_ProcessTiles(const pugi::xml_node& tilesetNode)
{
	...
	Resource<sf::Image>::type sourceImage = MultiResCache::getInstance().GetHandle<sf::Image>(imagePath);

	//add transparency mask from colour if it exists
	if(imageNode.attribute("trans"))
		sourceImage->get().createMaskFromColor(m_ColourFromHex(imageNode.attribute("trans").as_string()));

	//store image as a texture for drawing with vertex array
	sf::Texture tileset;
	tileset.loadFromImage(sourceImage->get());
	...
	//slice into tiles
	int columns = (sourceImage->get().getSize().x - margin) / (tileWidth + spacing);
	int rows = (sourceImage->get().getSize().y - margin) / (tileHeight + spacing);
...
bool MapLoader::m_ParseObjectgroup(const pugi::xml_node& groupNode, EventDispatcher& dispatch)
{
	...
	while(objectNode)
	{
		...
		if(objectNode.attribute("width") && objectNode.attribute("height"))
		{
			...
			if(objectNode.child("ellipse"))
			{
				...
				for(float angle = 0.f; angle < tau; angle += step)
				{
					...
					//shape.addPoint(IsometricToOrthogonal(point));
		...
		if(objectNode.attribute("rotation")) go->setRotation(objectNode.attribute("rotation").as_float());
		...
		if(objectNode.attribute("gid"))
		{
			...
			tile.sprite.setRotation(go->getRotation());
			tile.sprite.setColor(sf::Color(255u, 255u, 255u, static_cast<sf::Uint8>(255.f * layer.opacity)));
			tile.sprite.setPosition(go->getPosition());
			tile.gridCoord = sf::Vector2i(static_cast<int>(go->getPosition().x / m_tileWidth), static_cast<int>(go->getPosition().y / m_tileHeight));
...
bool MapLoader::m_ParseImageLayer(const pugi::xml_node& imageLayerNode)
{
	...
	Resource<sf::Image>::type image = MultiResCache::getInstance().GetHandle<sf::Image>(imageName);

	//set transparency if required
	if(imageNode.attribute("trans"))
	{
		image->get().createMaskFromColor(m_ColourFromHex(imageNode.attribute("trans").as_string()));
	}
