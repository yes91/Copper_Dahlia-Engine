...
bool MapLoader::Load(const std::string& map, EventDispatcher& dispatch)
{
	...
	while(currentNode)
	{
		...
		{
			if(!(m_mapLoaded = m_ParseObjectgroup(currentNode, dispatch)))
			{
				m_Unload();
				return false;
			}
...
std::vector<std::shared_ptr<GameObject>> MapLoader::QueryQuadTree(const sf::FloatRect& testArea)
{
	...
	case MapLayer::Debug:
		...
		for(auto& layer : m_layers)
		{
			if(layer.type == ObjectGroup)
			{
				...
				{
					if(!object->has<MapObjectComponent>()) continue;
					object->get<MapObjectComponent>().DrawDebugShape(rt, sf::RenderStates::Default);
				}
			}
		}
