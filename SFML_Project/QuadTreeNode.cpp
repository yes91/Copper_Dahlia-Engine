...
std::vector<std::shared_ptr<GameObject>> QuadTreeNode::Retrieve(const sf::FloatRect& bounds, sf::Uint16& searchDepth)
{	
	searchDepth = m_level;
	...
	else
	{
		...
		for(auto child = m_children.begin(); child != m_children.end(); ++child)
		{
			...
			{
				std::vector<std::shared_ptr<GameObject>> childObjects = node.Retrieve(bounds, searchDepth);
...
void QuadTreeNode::Insert(std::shared_ptr<GameObject> object)
{	
	//check if an object falls completely outside a node
	if(!object->get<IPhysicsComponent>().getGlobalBounds().intersects(m_bounds)) return;
	
	//if node is already split add object to corresponding child node
	//if it fits
	if(!m_children.empty())
	{
		sf::Int16 index = m_GetIndex(object->get<IPhysicsComponent>().getGlobalBounds());
		if(index != -1)
		{
	...
	if(m_objects.size() > MAX_OBJECTS && m_level < MAX_LEVELS)
	{
		...
		{
			sf::Int16 index = m_GetIndex(m_objects[i]->get<IPhysicsComponent>().getGlobalBounds());
			if(index != -1)
			{
