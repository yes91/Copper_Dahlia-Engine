...
namespace tmx
{
	///Creates a node used to build quad trees for spatial partitioning of GameObjects///
	...
	public:
		...
		std::vector<std::shared_ptr<GameObject>> Retrieve(const sf::FloatRect& bounds, sf::Uint16& currentDepth);
	...
	protected:
		...
		std::vector<std::shared_ptr<GameObject>> m_objects; //objects contained in current node
	...
	public:
		...
		std::vector<std::shared_ptr<GameObject>> Retrieve(const sf::FloatRect& bounds)
		{
